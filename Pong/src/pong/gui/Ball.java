...
public class Ball extends PongItem {
	...
	public void racketCollision(Point initialPos) {
		...
		if (pos.x <= Pong.getPlayers()[0].getRacket().getWidth()
				&& pos.x >= Pong.getPlayers()[0].getRacket().getPosition().getX()
				&& pos.y >= Pong.getPlayers()[0].getRacket().getPosition().getY()
				&& pos.y <= Pong.getPlayers()[0].getRacket().getPosition().getY()+Pong.getPlayers()[0].getRacket().getHeight()
				&& getBall_speed().getX()<0)
		{
			if(pos.y <= Pong.getPlayers()[0].getRacket().getPosition().getY()+(Pong.getPlayers()[0].getRacket().getHeight()*0.15)){
			...
			else if(pos.y >= Pong.getPlayers()[0].getRacket().getPosition().getY()+(Pong.getPlayers()[0].getRacket().getHeight()*0.85)){
				if(getBall_speed().getY()<0){
					setBall_speed(new Point(-getBall_speed().x, -getBall_speed().y));
				}
				else{
